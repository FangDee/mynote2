...
- 总结: redis是一个内存型的数据库
  ...
  数据类型 : strings  、hashs、 lists、  sets、  sorted set。

  redis数据持久化

  redis 哨兵 副本集  redis 集群
...
- Redis单线程单进程(6.x版本之前  6.x版本之后全部多线程)
...
```markdown
# 12.开启redis远程连接权限
- 从redis源码包中将redis.conf配置文件复制到安装目录/usr/redis/bin
- cp redis.conf /usr/redis/bin
- 编辑配置文件
- vi redis.conf
```

![image-20210909201953303](Redis.assets/image-20210909201953303.png)

```markdown
- 加载配置文件启动
- ./redis-server redis.conf
`注意: 开启远程连接后必须关闭网络防火墙或者开发对应端口`
```

### 7.2 Docker安装

```markdown
# 1.访问docker hub 获取指定版本redis镜像
...
- `注意:`**0-15 编号数据库 总数默认为16个数据库,默认通过客户端连接使用0号库**
...
list.add(user)  //0 user

list.add(user1) //1,user1
...
Value: [aa 0,bb 7,ee 8,cc 90,dd 80]
...
- 客户端方式: BGSAVE 和 SAVE指令  手动方式生成快照
- 服务器配置自动触发
...
spring data 技术:  操作关系型数据  非关系数据库

springboot 整合 redis = springboot + spring data + redis



Spring Data Redis: 两个对象  RedisTemplate、StringRedisTemplate

RedisTemplate： 是对对象的友好操作      key object  value object

StringRedisTemlate: 是对字符串友好操作  key value 都是字符串
...
```markdown
# 1.准备3台机器 [基于docker方式搭建]

## 1).基于命令方式搭建
- a.启动三个容器
  - docker run -d -p 6380:6379 redis:6.2.5  master
  - docker run -d -p 6381:6379 redis:6.2.5  slave01
  - docker run -d -p 6382:6379 redis:6.2.5  slave02
- b.分别进入3个容器
	- 在slave01  slave02中执行如下命令
	- slaveof  主节点ip 主节点端口

## 2).基于redis配置文件方式搭建 [推荐]
- a.创建3个目录
 - mkdir 6380 6381 6382

- b.在每个目录中放入dockerfile 和 redis.conf
	- dockerfile文件内容一致
	FROM redis:6.2.5
  COPY redis.conf /usr/local/etc/redis/redis.conf
  CMD ["redis-server", "/usr/local/etc/redis/redis.conf"]

- c.6380目录使用默认redis.conf构建镜像

- d.6381目录中redis.conf文件中在477行加入
	- replicaof 172.16.91.9(master) 6380(master端口)

- e.6382目录中redis.conf文件中在477行加入
	- replicaof 172.16.91.9(master) 6380(master端口)

- f.基于以上构建三个镜像,分别运行容器,启动之后就是主从架构
	- docker run -d -p 6380:6379 redis-master:1.0
	- docker run -d -p 6381:6379 redis-slave01:1.0
	- docker run -d -p 6382:6379 redis-slave02:1.0
```



```markdown
# 1.准备3台机器并修改配置 [基于安装包方式搭建]
- master
	port 6379
	bind 0.0.0.0
...
spring.redis.sentinel.master=aa
...
```markdown
# 1.准备环境安装ruby以及redis集群依赖[4.x老版本]
